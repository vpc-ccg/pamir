...
LFLAGS = $(LDFLAGS) -lm -lz -L$(SPOA_LIB) -lspoa -no-pie -lpthread -lssl
#LFLAGS = -lm -lz -no-pie
.PHONY: default_make
...
.PHONY: all
all: $(EDLIB_HED_PATH) $(EDLIB_SRC_PATH) $(LOGGER_HED_PATH) $(CEREAL_HED_PATH) $(SPOA_HED_PATH) $(SPOA_LIB) $(CLASP_HED_PATH) $(BIN_PATH)/$(EXE)  $(COPIED_SCRIPTS)
...
$(BUILD_PATH)/clasp/%.o: $(CLASP_EXT_SRC_PATH)/%.c
	gcc -w -pedantic -std=c99 -O3 -DPROG3NFO -DDBGLEVEL=0 $(INCLUDES) -c $< -o $@