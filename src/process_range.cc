...
#define INSERT_MIN_CONSIDER_PERCENT 0.6
    ...
    sup_type remap_if_on_breakpoint(sam_record &rec, char *seq, int flank_len, int insert_len){
        ...
        EdlibAlignResult result = edlibAlign(rec.seq.c_str(), rec.seq.size(), seq, ref_len, edlibNewAlignConfig(-1, EDLIB_MODE_HW, EDLIB_TASK_PATH, NULL, 0));
...
//                    move_len -= cigar_len;
            ...
            if(cigar_iter==rec.cgr.rend()){
                if(what_is_this_cigar(new_cigar_array.back().second) == cigar_character_type::softclip && move_len>0){
                    pair<int,char> last_pair = new_cigar_array.back();
                    new_cigar_array.pop_back();
...
//    move_len -= cigar_len;
    ...
    sup_type remap_unmapped(sam_record &read, const string &chr_name, char *seq, int flank_len, int insert_len){
        ...
        int edlib_K = 10;
        int reversed = 0;
        EdlibAlignResult result = edlibAlign(read.seq.c_str(), read.seq.size(), seq, seq_len, edlibNewAlignConfig(edlib_K, EDLIB_MODE_HW, EDLIB_TASK_PATH, NULL, 0));
        if(result.numLocations == 0){
            char *revseq = (char *) malloc(read.seq.size());
            reverseComplete((char *)read.seq.c_str(), revseq ,read.seq.size());
            result = edlibAlign(revseq, read.seq.size(), seq, seq_len, edlibNewAlignConfig(edlib_K, EDLIB_MODE_HW, EDLIB_TASK_PATH, NULL, 0));
    ...
    int main(int argc, char **argv){
        ...
        for(auto p : reads){ // p is a pair<string, vector<string>>
            ...
            if( read_pair.size() == 1){ //Single End
                ...
                int min_pos = std::min({read_pair[0].pos, read_pair[0].end_pos, read_pair[1].pos, read_pair[1].end_pos},[](int a, int b){return a < b;});
                int max_pos = std::max({read_pair[0].pos, read_pair[0].end_pos, read_pair[1].pos, read_pair[1].end_pos},[](int a, int b){return a < b;});
